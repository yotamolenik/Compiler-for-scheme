...
try
  ...
  let code = (file_to_string "gabilib.scm")  ^ (file_to_string infile) in
  ...
  let code_fragment = String.concat "\n\n"
                        (List.map
                           (fun ast -> break_unique:= !break_unique + 1; (generate ast) ^ "\n\tcall write_sob_if_not_void\nbreak_" ^ string_of_int(!break_unique) ^ ":\n")
                           asts) in
