...
module Prims : PRIMS = struct
  ...
  mov rax, SOB_VOID_ADDRESS", make_binary, "set_cdr";
   ...
   (* apply procedure obj ... list -> rsi,rdi,rdx*)
"mov rbx, rsi                             ;; closure function to call
;; push everything in order
;; shift frame
mov rcx, qword [rbp + 3*WORD_SIZE]     ;; n+2             
mov r8, rcx
mov rcx, qword [rbp+ (r8+3)*WORD_SIZE] ;; list in rcx
mov rdi, 0                             ;; rdi will be length
mov r9, SOB_NIL_ADDRESS
loop_length:
;; CAR rdx, rcx                           ;; each time check if car is nil
 cmp rcx, SOB_NIL_ADDRESS
 je end_loop_length
 inc rdi
 CAR rdx, rcx
 MAKE_PAIR(r10,rdx,r9)
 mov r9, r10
 CDR rcx, rcx
 jmp loop_length
 
end_loop_length:                        ;; (ym,.... y1) in r9, length in rdi
...
mov rcx, qword [rbp + 3*WORD_SIZE]     ;; n+2
sub rcx, 2                             ;; no list, and do until 1 so no f
push_args:
  cmp rcx, 0
  je end_push_args
  push PVAR(rcx)
  ...
  jmp push_args
end_push_args:
add rdi, qword [rbp + 3*WORD_SIZE]
sub rdi, 2
push rdi                              ;; new number of params
CDR rcx, rbx                          ;; env in rcx
push qword rcx
push qword [rbp+1*WORD_SIZE]          ;; push ret, dont push rbp

SHIFT_FRAME 200                       ;; probably too much but it doesnt matter, we just move junk around
mov rcx, [rbp+3*WORD_SIZE]            ;; n
sub rcx, rdi
inc rcx
shl rcx, 3
mov rsp, rbp
add rsp, rcx
mov rbp, rsp                          ;; just in case
CDR rcx, rbx                          ;; code in rcx
jmp rcx
", make_unary,"apply";
      ] in
